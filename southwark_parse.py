"""
Parse saved Southwark licence HTML pages into a structured JSON dataset.

The script relies on HTML files captured by ``southwark_fetch.py`` and extracts
...
from southwark_scraper.config import SOUP_PARSER

from southwark_scraper.parse_southwark import (
...
DATA_ROOT = SCRAPE_ROOT / "data" / "southwark"
LOG_DIR = DATA_ROOT / "logs"
METADATA_PATH = DATA_ROOT / "metadata.json"
OUTPUT_PATH = DATA_ROOT / "southwark_licences.json"


def configure_logger() -> logging.Logger:
    LOG_DIR.mkdir(parents=True, exist_ok=True)
    logger = logging.getLogger("southwark.parse")
...
def find_reference(soup: BeautifulSoup) -> Optional[str]:
    pattern = re.compile(r"SWK-\d+", re.IGNORECASE)
