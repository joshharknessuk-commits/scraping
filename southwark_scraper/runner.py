"""End-to-end orchestration for scraping the Southwark register."""
...
from .config import COUNCIL_NAME, Settings, SnapshotMode, SOUP_PARSER
...
def _metadata_reference(entry: dict) -> Optional[str]:
    ...
    for key in ("detail_url", "additional_url", "detail_html", "additional_html"):
        ...
        if isinstance(value, str):
            extracted = parse_reference_from_text(value)
            if extracted:
                return extracted
    return None
...
def run(postcodes: Iterable[str], settings: Settings) -> List[LicenceRecord]:
    """Scrape the Southwark register for the supplied postcodes."""
    postcodes = list(postcodes)
    rng = random.Random(settings.seed)
    rng.shuffle(postcodes)

    paths = ensure_output_paths(settings.output_root)
    logger = get_logger("southwark.scraper", paths.log_dir, settings.log_level)

    logger.info(
        "Starting Southwark scrape for %s postcodes (concurrency=%s, snapshots=%s, jsonl=%s)",
        len(postcodes),
