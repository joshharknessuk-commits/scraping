...
AdditionalUrlResolver = Callable[[BeautifulSoup, str], Optional[str]]
...
def fetch_pages(
    ...
    force: bool,
    resolve_additional_url: AdditionalUrlResolver,
    ...
    if (
        ...
        and settings.snapshot_mode == SnapshotMode.ALL
    ):
    ...
    additional_url = resolve_additional_url(detail_soup, detail_url)

    if additional_url:
        if (
            not force
            and additional_path is not None
            and settings.snapshot_mode == SnapshotMode.ALL
        ):
