"""Generic postcode search flow used by council scrapers."""
...
def search_postcode(
    ...
    *,
    ...
    try:
        _load_search_page(env, page, postcode, settings, base_url)
        env.throttle()
        ...
        while True:
            ...
            if not _wait_for_results(env, page, postcode, settings, result_container_selector):
                break
...
def _load_search_page(env: BrowserEnv, page, postcode: str, settings: BaseSettings, base_url: str) -> None:
    def _attempt() -> None:
        ...
        try:
            ...
            env.logger.debug("Timeout waiting for network idle on %s: %s", postcode, exc)

    env.with_retries(_attempt, label=f"load-base:{postcode}")


def _fill_search_form(env: BrowserEnv, page, postcode: str, settings: BaseSettings) -> None:
    try:
...
def _go_to_next_page(env: BrowserEnv, page, settings: BaseSettings) -> bool:
    ...
    def _attempt() -> None:
        ...
        try:
            ...
            env.logger.debug("Timed out waiting for network idle during pagination")
