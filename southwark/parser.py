"""Southwark-specific parsing helpers."""
...
from .config import BASE_URL, REFERENCE_PATTERN, SOUP_PARSER
...
def parse_search_results(html: str) -> List[ResultRow]:
    ...
    for heading in soup.select("h2"):
        ...
        reference = extract_reference(detail_href, REFERENCE_PATTERN) or extract_reference(
            anchor.get_text(" ", strip=True), REFERENCE_PATTERN
        )
        if not reference:
            continue

        anchor_text = anchor.get_text(" ", strip=True) or None
        if anchor_text and anchor_text.strip().lower() == detail_href.strip().lower():
            anchor_text = None
