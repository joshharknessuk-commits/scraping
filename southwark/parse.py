...
from common.config import SOUP_PARSER
...
from .config import BASE_URL, RESULT_LINK_PATTERN
...
def parse_search_results(html: str) -> List[ResultRow]:
    ...
    for heading in soup.select("h2"):
        ...
        reference = extract_reference(detail_href, pattern=RESULT_LINK_PATTERN.pattern) or extract_reference(
            anchor.get_text(" ", strip=True), pattern=RESULT_LINK_PATTERN.pattern
        )
        if not reference:
            continue
